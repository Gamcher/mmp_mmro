...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        for sentence in parallel_corpus:
            sentence_posterior = self.translation_probs[np.ix_(sentence.source_tokens, sentence.target_tokens)]
            sentence_posterior = sentence_posterior / np.sum(sentence_posterior, axis=0, keepdims=True)
            posteriors.append(sentence_posterior)
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for idx, sentence in enumerate(parallel_corpus):
            log_grid= np.log(self.translation_probs[np.ix_(sentence.source_tokens, sentence.target_tokens)] + 1e-8)
            const_part = np.log(len(sentence.source_tokens) * (posteriors[idx] + 1e-8))

            elbo += np.sum(posteriors[idx] * (log_grid - const_part))
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for idx, sentence in enumerate(parallel_corpus):
            s_idx, t_idx = np.meshgrid(np.arange(len(sentence.source_tokens)), np.arange(len(sentence.target_tokens)), indexing='ij')
        
            global_src = sentence.source_tokens[s_idx.ravel()]
            global_trg = sentence.target_tokens[t_idx.ravel()]

            np.add.at(
                self.translation_probs, 
                (global_src, global_trg), 
                posteriors[idx].ravel()
            )
    ...
    def align(self, sentences):
        result = []
        for sentence in sentences:
            index_source = np.argmax(self.translation_probs[np.ix_(sentence.source_tokens, sentence.target_tokens)], axis=0)
            alignment = [(t + 1, s + 1) for s, t in enumerate(index_source)]
            result.append(alignment)
        return result
